...
import { setsWon } from "./rules";
import { useBolaTampar } from "./store";
import { DISCIPLINE_LABEL, formatSets, wilayahName, type BolaTamparMatch } from "./types";

function courtOrder(match: BolaTamparMatch) {
  return match.courtId ?? match.court ?? "";
}

export default function ProjectionBoard() {
  const { matches } = useBolaTampar();
  ...
  const live = useMemo(
    () =>
      matches
        ...
        .sort((a, b) => courtOrder(a).localeCompare(courtOrder(b))),
    [matches],
  );
  ...
  return (
    <ProjectionShell subtitle="Live bola tampar" idle={idle} onExit={exitProjection}>
...
function ProjectionCard({ match, large }: { match: BolaTamparMatch; large: boolean }) {
  const won = setsWon(match.games);
  ...
  return (
    <section className="flex min-h-0 flex-col justify-center rounded-[28px] bg-[var(--surface)] px-8 py-8">
      <p className={`text-center text-[var(--muted)] ${large ? "text-xl" : "text-base"}`}>
        {match.court} · {DISCIPLINE_LABEL[match.discipline]}
      </p>
      <div className="mt-6 grid items-center gap-4 sm:grid-cols-[1fr_auto_1fr]">
        <div className="min-w-0 text-center sm:text-right">
          <p className={`font-medium ${large ? "text-3xl" : "text-xl"}`}>{wilayahName(match.sideA.wilayahId)}</p>
        </div>
        <div className="text-center">
          ...
          <p className={`tabular mt-2 text-[var(--muted)] ${large ? "text-xl" : "text-sm"}`}>Sets</p>
        ...
        <div className="min-w-0 text-center sm:text-left">
          <p className={`font-medium ${large ? "text-3xl" : "text-xl"}`}>{wilayahName(match.sideB.wilayahId)}</p>
        </div>
      </div>
      <p className={`tabular mt-8 text-center text-[var(--lime)] ${large ? "text-3xl" : "text-xl"}`}>
        {formatSets(match.games) || "0–0"}
      </p>
    </section>
  );
}
...
export async function enterProjection() {
  window.location.hash = "/project/bolatampar";
  try {
