...
import { DISCIPLINE_LABEL, formatSets, volleyballCourtName, wilayahName, type BolaTamparMatch } from "./types";

type View = { page: "list" } | { page: "new" } | { page: "sheet"; id: string };

function statusLabel(match: BolaTamparMatch): string {
  if (match.status === "scheduled") return "Scheduled";
  if (match.status === "complete") return "Final";
  return "Live";
}

export default function BolaTamparScorer() {
  ...
  return (
    <>
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          <p className="text-sm text-[var(--lime)]">{volleyballCourtName(court.name)}</p>
          <h1 className="text-xl font-semibold">Bola Tampar scorer</h1>
        </div>
      ...
      {rows.length === 0 ? (
        <p className="rounded-2xl bg-[var(--surface)] px-5 py-12 text-center text-[var(--muted)]">
          No bola tampar matches on {volleyballCourtName(court.name)} yet.
        </p>
      ) : (
          ...
          {rows.map((match) => {
            ...
            return (
              <li key={match.id}>
                ...
                >
                  <span className="text-sm text-[var(--muted)]">{DISCIPLINE_LABEL[match.discipline]}</span>
                  ...
                  <span className="flex items-center justify-between gap-3">
                    <span className="tabular text-[var(--lime)]">
                      {match.games.length > 0 ? `${won.a}–${won.b} · ${formatSets(match.games)}` : "Not started"}
                    </span>
