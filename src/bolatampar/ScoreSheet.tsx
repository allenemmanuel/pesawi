...
import { sheetHeader, wilayahName } from "./types";
...
export default function ScoreSheet({ matchId, onBack }: Props) {
  const { getMatch, saveGames, completeMatch } = useBolaTampar();
  ...
  function hint() {
    ...
    const index = analysis?.inProgressIndex;
    if (index != null) {
      const target = pointsToWinSet(index);
      return `Set ${index + 1} is in progress. First to ${target}, win by ${MUST_WIN_BY}. Complete when one side has ${GAMES_TO_WIN_MATCH} sets.`;
    }
    return `Sets 1–4 first to ${POINTS_TO_WIN_SET}, Set 5 to ${POINTS_TO_WIN_DECIDING_SET}, win by ${MUST_WIN_BY}. Complete when one side has ${GAMES_TO_WIN_MATCH} sets.`;
  }
    ...
    <div className="max-w-3xl">
      ...
      <div className="overflow-hidden rounded-[28px] bg-[var(--surface)]">
        <div className="grid grid-cols-[5rem_1fr_1fr] border-b border-[var(--line)] px-4 py-4 text-center text-sm text-[var(--muted)] sm:grid-cols-[7rem_1fr_1fr] sm:px-6">
          <span />
          <span className="font-medium text-[var(--text)]">{wilayahName(match.sideA.wilayahId)}</span>
          <span className="font-medium text-[var(--text)]">{wilayahName(match.sideB.wilayahId)}</span>
        </div>
          ...
          return (
            ...
            >
              <p className="text-sm text-[var(--muted)]">
                Set {index + 1}
                {index === MAX_SETS - 1 ? (
                  <span className="block text-xs">to {target}</span>
                ) : null}
              </p>
              <ScoreInput
                value={slots[index]?.a ?? ""}
