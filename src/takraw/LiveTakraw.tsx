...
import { buildTakrawRoundRobin } from "./standings";
import { useTakraw } from "./store";

export default function LiveTakraw() {
  const { matches, setFinalScore, clearFinalScore } = useTakraw();
  const { court } = useSession();

  const rows = useMemo(() => buildTakrawRoundRobin(matches), [matches]);
  ...
  return (
    <SportLiveBoard sport="Sepak Takraw">
      <RoundRobinTable
        rows={rows}
