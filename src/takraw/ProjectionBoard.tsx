...
import { formatSets, sideLine, wilayahName, type TakrawMatch } from "./types";

function courtOrder(match: TakrawMatch) {
  return match.courtId ?? match.court ?? "";
}

export default function ProjectionBoard() {
  const { matches } = useTakraw();
  ...
  return (
    <ProjectionShell subtitle="Live sepak takraw" idle={idle} onExit={exitProjection}>
...
function ProjectionCard({ match, large }: { match: TakrawMatch; large: boolean }) {
...
export async function enterProjection() {
  window.location.hash = "/project/takraw";
  try {
