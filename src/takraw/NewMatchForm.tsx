...
export default function NewMatchForm({ onCancel, onCreated }: Props) {
  const { addMatch } = useTakraw();
  const { court } = useBadminton();
  const [discipline, setDiscipline] = useState<Discipline>("MR");
  const count = playersPerSide(discipline);
  const [sideA, setSideA] = useState<SideSnapshot>(() => emptySide("kota-kinabalu", 3));
  const [sideB, setSideB] = useState<SideSnapshot>(() => emptySide("tawau", 3));
