...
export default function NewMatchForm({ onCancel, onCreated }: Props) {
  const { addMatch } = usePickleball();
  const { court } = useBadminton();
  const [discipline, setDiscipline] = useState<Discipline>("WD");
  const count = playersPerSide(discipline);
  const [sideA, setSideA] = useState<SideSnapshot>(() => emptySide("kota-kinabalu", 2));
  const [sideB, setSideB] = useState<SideSnapshot>(() => emptySide("tawau", 2));
    ...
    <form onSubmit={onSubmit} className="pb-8">
      ...
      <p className="mb-4 rounded-2xl bg-[var(--surface)] px-4 py-3 text-sm text-[var(--muted)]">
        ...
        <span className="font-medium text-[var(--text)]">
          {court ? pickleballCourtName(court.name) : "this court"}
        </span>
      </p>
