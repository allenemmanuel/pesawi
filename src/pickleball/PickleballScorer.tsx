...
import { formatGames, pickleballCourtName, sideLine, wilayahName, type PickleballMatch } from "./types";

type View = { page: "list" } | { page: "new" } | { page: "sheet"; id: string };

function statusLabel(match: PickleballMatch): string {
  ...
  return (
    <>
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          <p className="text-sm text-[var(--lime)]">{pickleballCourtName(court.name)}</p>
          <h1 className="text-xl font-semibold">Pickleball scorer</h1>
        </div>
      ...
      {rows.length === 0 ? (
        <p className="rounded-2xl bg-[var(--surface)] px-5 py-12 text-center text-[var(--muted)]">
          No pickleball matches on {pickleballCourtName(court.name)} yet.
        </p>
      ) : (
