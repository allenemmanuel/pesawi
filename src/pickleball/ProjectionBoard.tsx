...
import { formatGames, sideLine, wilayahName, type PickleballMatch } from "./types";

function lastGame(match: PickleballMatch) {
  return match.games[match.games.length - 1] ?? null;
}

function courtOrder(match: PickleballMatch) {
  return match.courtId ?? match.court ?? "";
}

export default function ProjectionBoard() {
  const { matches } = usePickleball();
  ...
  return (
    <ProjectionShell subtitle="Live pickleball" idle={idle} onExit={exitProjection}>
...
function ProjectionCard({ match, large }: { match: PickleballMatch; large: boolean }) {
...
export async function enterProjection() {
  window.location.hash = "/project/pickleball";
  try {
