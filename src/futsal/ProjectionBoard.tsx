...
export default function ProjectionBoard() {
  ...
  return (
    <ProjectionShell subtitle="Live futsal" idle={idle} onExit={exitProjection}>
...
function ProjectionCard({ match, large }: { match: FutsalMatch; large: boolean }) {
  const note = publicScoreNote(match);
  ...
  return (
    <section className="flex min-h-0 flex-col justify-center rounded-[28px] bg-[var(--surface)] px-8 py-8">
      <p className={`text-center text-[var(--muted)] ${large ? "text-xl" : "text-base"}`}>
        {match.court} · {DISCIPLINE_LABEL[match.discipline]} · {PHASE_LABEL[match.phase]}
      </p>
      <div className="mt-6 grid items-center gap-4 sm:grid-cols-[1fr_auto_1fr]">
        ...
        <div className="text-center">
          ...
          <p className={`mt-2 text-[var(--muted)] ${large ? "text-xl" : "text-sm"}`}>
            {note ?? publicPeriodLabel(match.period)}
          </p>
...
export async function enterProjection() {
  window.location.hash = "/project/futsal";
  try {
