import { WILAYAH, wilayahChipStyle, type FutsalMatch } from "./types";
import { buildRoundRobin } from "./standings";
import { latestUpdatedAt } from "../lib/lastUpdated";
import LastUpdated from "../standings/LastUpdated";
import ScoreCellInput from "../standings/ScoreCellInput";
...
const STAT_HEADERS: { key: string; label: string; title: string }[] = [
  { key: "m", label: "M", title: "Menang" },
  { key: "s", label: "S", title: "Seri" },
  { key: "k", label: "K", title: "Kalah" },
  { key: "j", label: "J", title: "Jaringan" },
  { key: "b", label: "B", title: "Bolos" },
  { key: "pj", label: "PJ", title: "Perbezaan Jaringan" },
  { key: "mata", label: "MATA", title: "Mata" },
];

export default function RoundRobin({ matches, title, editable = false, onCommitScore }: Props) {
  const rows = buildRoundRobin(matches);
  const lastUpdated = latestUpdatedAt(matches);
  ...
  return (
    <section className="w-full min-w-0">
      ...
      <div className="live-card-enter w-full min-w-0 overflow-x-auto overscroll-x-contain rounded-2xl border border-[var(--line)] bg-[rgba(5,9,20,0.78)] shadow-[0_12px_40px_rgba(0,0,0,0.35)] backdrop-blur-sm [-webkit-overflow-scrolling:touch]">
        <table className="w-full min-w-[58rem] table-fixed border-collapse text-xs sm:text-sm">
          <colgroup>
            ...
            {STAT_HEADERS.map((col) => (
              <col key={col.key} className="w-[2.75rem]" />
            ))}
            ...
            <tr className="border-b border-[var(--line)]">
              ...
              {STAT_HEADERS.map((col) => (
                <th
                  key={col.key}
                  title={col.title}
                  className={`px-1 py-3 text-center font-semibold tracking-wide sm:px-1.5 sm:py-4 ${goldCol}`}
                >
                  {col.label}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.wilayah.id} className="border-b border-[var(--line)] last:border-b-0">
                ...
                <td className={`px-1 py-3 text-center tabular font-medium sm:py-4 ${goldCol}`}>{row.wins}</td>
                <td className={`px-1 py-3 text-center tabular font-medium sm:py-4 ${goldCol}`}>{row.draws}</td>
                <td className={`px-1 py-3 text-center tabular font-medium sm:py-4 ${goldCol}`}>{row.losses}</td>
                <td className={`px-1 py-3 text-center tabular font-medium sm:py-4 ${goldCol}`}>{row.goalsFor}</td>
                <td className={`px-1 py-3 text-center tabular font-medium sm:py-4 ${goldCol}`}>{row.goalsAgainst}</td>
                <td className={`px-1 py-3 text-center tabular font-medium sm:py-4 ${goldCol}`}>
                  {row.goalDiff > 0 ? `+${row.goalDiff}` : row.goalDiff}
                </td>
                <td className={`px-1 py-3 text-center tabular font-semibold sm:py-4 ${goldCol}`}>{row.points}</td>
