...
import { DISCIPLINE_LABEL, PHASE_LABEL, publicPeriodLabel, publicScoreNote, wilayahName, type FutsalMatch } from "./types";

type View = { page: "list" } | { page: "new" } | { page: "sheet"; id: string };

function statusLabel(match: FutsalMatch): string {
  if (match.status === "scheduled") return "Scheduled";
  if (match.status === "complete") return "Final";
  return "Live";
}

export default function FutsalScorer() {
    ...
    <>
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl font-semibold">Futsal scorer</h1>
        </div>
      ...
      {rows.length === 0 ? (
        <p className="rounded-2xl bg-[var(--surface)] px-5 py-12 text-center text-[var(--muted)]">
          No futsal matches on {court.name} yet.
        </p>
      ) : (
              ...
              <li key={match.id}>
                ...
                >
                  <span className="text-sm text-[var(--muted)]">
                    {DISCIPLINE_LABEL[match.discipline]} · {PHASE_LABEL[match.phase]}
                  </span>
                  ...
                  <span className="flex items-center justify-between gap-3">
                    <span className="tabular text-[var(--lime)]">
                      ...
                      {note ? ` · ${note}` : ` · ${publicPeriodLabel(match.period)}`}
                    </span>
