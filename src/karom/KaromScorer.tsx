...
import { boardNameFromCourt, formatGames, sideLine, wilayahName, type KaromMatch } from "./types";

type View = { page: "list" } | { page: "new" } | { page: "sheet"; id: string };

function statusLabel(match: KaromMatch): string {
  if (match.status === "scheduled") return "Scheduled";
  if (match.status === "complete") return "Final";
  return "Live";
}

export default function KaromScorer() {
  ...
  const rows = useMemo(
    () =>
      matches.filter((match) => {
        ...
        if (match.boardId !== court.id) return false;
  ...
  return (
    <>
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          <p className="text-sm text-[var(--lime)]">{boardNameFromCourt(court.name)}</p>
          <h1 className="text-xl font-semibold">Karom scorer</h1>
        </div>
      ...
      {rows.length === 0 ? (
        <p className="rounded-2xl bg-[var(--surface)] px-5 py-12 text-center text-[var(--muted)]">
          No karom matches on {boardNameFromCourt(court.name)} yet.
        </p>
      ) : (
