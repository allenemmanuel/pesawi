...
export function fetchMatches() {
  return listSportMatches<KaromMatch>("Karom");
}

export function subscribeMatches(onChange: (matches: KaromMatch[]) => void) {
  return subscribeSportMatches<KaromMatch>("Karom", onChange);
}
...
export async function createMatch(draft: {
  ...
  const match: KaromMatch & { courtId: string } = {
    id: newMatchId("kr"),
    sport: "Karom",
...
export async function patchMatchGames(id: string, games: GameScore[]) {
  const session = await requireCourt();
  if (!session.ok) return fail(session.error, 401);
  const current = await readMatch<KaromMatch>(id);
  ...
  try {
    const match = await patchMatch<KaromMatch>(id, {
      games,
...
export async function postCompleteMatch(id: string, games: GameScore[]) {
  ...
  try {
    const match = await patchMatch<KaromMatch>(id, { games, status: "complete" });
    return ok({ match });
  } catch (error) {
    return fail(writeError(error), 403);
  }
...
export async function setFinalScore(sideAId: string, sideBId: string, scoreA: number, scoreB: number) {
  ...
  const listed = await listSportMatches<KaromMatch>("Karom");
  ...
  try {
    if (existing) {
      ...
      const match = await patchMatch<KaromMatch>(existing.id, { games, status: "complete" });
      return ok({ match });
    }
...
export async function clearFinalScore(sideAId: string, sideBId: string) {
  return clearSportPairResult<KaromMatch>("Karom", sideAId, sideBId);
}
