import { playersPerSide, type Discipline, type GameScore, type KaromMatch, type SideSnapshot } from "./types";

export const POINTS_TO_WIN_GAME = 25;
/** Max from one board: 1 per opponent man left, +3 queen if still on 21 or under. */
const MAX_BOARD_POINTS = 12;
export const MAX_GAME_POINTS = 8 * MAX_BOARD_POINTS;

export function isLegalFinished(a: number, b: number): boolean {
  if (!Number.isInteger(a) || !Number.isInteger(b) || a < 0 || b < 0) return false;
  if (a === b) return false;
  const hi = Math.max(a, b);
  return hi >= POINTS_TO_WIN_GAME && hi <= MAX_GAME_POINTS;
}

export function isInProgress(a: number, b: number): boolean {
  if (!Number.isInteger(a) || !Number.isInteger(b) || a < 0 || b < 0) return false;
  return Math.max(a, b) < POINTS_TO_WIN_GAME;
}
...
export function liveStatus(games: GameScore[]): "scheduled" | "live" {
  ...
  if (games.length === 1 && games[0].a === 0 && games[0].b === 0) return "scheduled";
  return "live";
}
...
export function analyzeGames(games: GameScore[]): MatchAnalysis {
  ...
  if (games.length > 3) {
    issues.push("A match has at most 3 games.");
  }

  const sliced = games.slice(0, 3);
  let wonA = 0;
  let wonB = 0;
  ...
  for (let i = 0; i < sliced.length; i += 1) {
    ...
    if (isLegalFinished(game.a, game.b)) {
      if (matchWonAt < 0) {
  ...
  const game3Enabled = splitAfterTwo;
  const winner = wonA >= 2 ? "a" : wonB >= 2 ? "b" : null;
...
export function analyzeMatch(match: KaromMatch): MatchAnalysis {
