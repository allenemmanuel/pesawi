...
import { formatGames, sideLine, wilayahName, type KaromMatch } from "./types";

function lastGame(match: KaromMatch) {
  return match.games[match.games.length - 1] ?? null;
}

function boardOrder(match: KaromMatch) {
  return match.boardId ?? match.board ?? "";
}

export default function ProjectionBoard() {
  const { matches } = useKarom();
  ...
  return (
    <ProjectionShell subtitle="Live karom" idle={idle} onExit={exitProjection}>
...
function ProjectionCard({ match, large }: { match: KaromMatch; large: boolean }) {
...
export async function enterProjection() {
  window.location.hash = "/project/karom";
  try {
