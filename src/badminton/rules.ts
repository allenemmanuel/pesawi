import type { BadmintonMatch, GameScore } from "./types";
...
export function isLegalFinished(a: number, b: number): boolean {
  ...
  if (hi >= 22 && hi <= 29 && lo === hi - 2) return true;
  if (hi === 30 && (lo === 28 || lo === 29)) return true;
  return false;
}
...
export function isInProgress(a: number, b: number): boolean {
  ...
  if (hi < 21) return true;
  if (hi >= 21 && hi <= 29 && lead <= 1) return true;
  return false;
}
...
export function analyzeMatch(match: BadmintonMatch): MatchAnalysis {
