...
export default function ProjectionBoard() {
  ...
  return (
    <ProjectionShell subtitle="Live badminton" idle={idle} onExit={exitProjection}>
      ...
      {live.length === 0 ? (
        ...
        <div
          className={`grid flex-1 gap-5 px-6 pb-6 ${many ? "grid-cols-2" : "grid-cols-1"}`}
        >
          {live.map((match) => (
...
function ProjectionCard({ match, large }: { match: BadmintonMatch; large: boolean }) {
  ...
  return (
    <section className="flex min-h-0 flex-col justify-center rounded-[28px] bg-[var(--surface)] px-8 py-8">
      <p className={`text-center text-[var(--muted)] ${large ? "text-xl" : "text-base"}`}>
        {match.court} · {match.discipline} · {CATEGORY_LABEL[match.category]}
      </p>
      <div className="mt-6 grid items-center gap-4 sm:grid-cols-[1fr_auto_1fr]">
        <div className="min-w-0 text-center sm:text-right">
          <p className={large ? "text-xl text-[var(--muted)]" : "text-sm text-[var(--muted)]"}>
            {wilayahName(match.sideA.wilayahId)}
          </p>
          <p className={`mt-1 font-medium ${large ? "text-3xl" : "text-xl"}`}>{pairLine(match.sideA)}</p>
        </div>
        ...
        <div className="min-w-0 text-center sm:text-left">
          <p className={large ? "text-xl text-[var(--muted)]" : "text-sm text-[var(--muted)]"}>
            {wilayahName(match.sideB.wilayahId)}
          </p>
          <p className={`mt-1 font-medium ${large ? "text-3xl" : "text-xl"}`}>{pairLine(match.sideB)}</p>
        </div>
      </div>
