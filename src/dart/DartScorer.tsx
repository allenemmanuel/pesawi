...
import { boardNameFromCourt, sideLine, wilayahName, type DartMatch } from "./types";

type View = { page: "list" } | { page: "new" } | { page: "sheet"; id: string };

function statusLabel(match: DartMatch): string {
    ...
    <>
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl font-semibold">Dart scorer</h1>
        </div>
      ...
      {rows.length === 0 ? (
        <p className="rounded-2xl bg-[var(--surface)] px-5 py-12 text-center text-[var(--muted)]">
          No dart matches on {boardNameFromCourt(court.name)} yet.
        </p>
      ) : (
