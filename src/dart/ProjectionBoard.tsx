...
import { LEGS_TO_WIN, START_SCORE, formatLabel } from "./rules";
import { useDart } from "./store";
import { sideLine, wilayahName, type DartMatch } from "./types";

function boardOrder(match: DartMatch) {
  return match.boardId ?? match.board ?? "";
}

export default function ProjectionBoard() {
  const { matches } = useDart();
  ...
  const live = useMemo(
    () =>
      matches
        ...
        .sort((a, b) => boardOrder(a).localeCompare(boardOrder(b))),
    [matches],
  );
  ...
  return (
    <ProjectionShell subtitle="Live dart" idle={idle} onExit={exitProjection}>
...
function ProjectionCard({ match, large }: { match: DartMatch; large: boolean }) {
  return (
    <section className="flex min-h-0 flex-col justify-center rounded-[28px] bg-[var(--surface)] px-8 py-8">
      ...
      <div className="mt-6 grid items-center gap-4 sm:grid-cols-[1fr_auto_1fr]">
        <div className="min-w-0 text-center sm:text-right">
          ...
          <p className={`tabular mt-2 font-semibold text-[var(--lime)] ${large ? "text-4xl" : "text-2xl"}`}>
            {match.remainingA}
          </p>
        </div>
        <div className="text-center">
          ...
          <p className={`mt-2 text-[var(--muted)] ${large ? "text-xl" : "text-sm"}`}>
            Legs · first to {LEGS_TO_WIN}
          </p>
          <p className={`mt-1 text-[var(--muted)] ${large ? "text-base" : "text-xs"}`}>
            Leg {match.currentLeg} · {START_SCORE} double-out
          </p>
      ...
      <p className={`mt-4 text-center text-[var(--muted)] ${large ? "text-sm" : "text-xs"}`}>{formatLabel()}</p>
    </section>
  );
}

export async function enterProjection() {
  window.location.hash = "/project/dart";
  try {
