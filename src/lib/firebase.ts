...
// NOTE: `||` (not `??`) is deliberate — it falls back to the bundled default when
// the env var is missing *or* an empty string. Hosts like Vercel can inject a
// declared-but-blank env var as "", which `??` would not catch, leaving Firebase
// with no config and crashing the app on load.
