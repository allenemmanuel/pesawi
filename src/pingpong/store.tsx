...
export type SaveResult = { ok: true; match: PingPongMatch } | { ok: false; error: string };

type PingPongStore = {
  matches: PingPongMatch[];
  ready: boolean;
  refresh: () => Promise<void>;
  getMatch: (id: string) => PingPongMatch | undefined;
  addMatch: (draft: {
...
const PingPongContext = createContext<PingPongStore | null>(null);
...
export function PingPongProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<PingPongStore>(
    () => ({
      matches,
      ready,
      refresh,
  ...
  return createElement(PingPongContext.Provider, { value }, children);
}

export function usePingPong(): PingPongStore {
  const store = useContext(PingPongContext);
  if (!store) {
